...
public class LowerBasketPose implements Action {
    ...
    public LowerBasketPose(ArmRotState rotState, ArmExtState extState, ElbowState elbowState, WristState wristState) {
        this.rotState = rotState;
        this.extState = extState;
        this.elbowState = elbowState;
        this.wristState = wristState;
        slidesIn = new SetExtPose(extState, ArmExtPose.RETRACT);
        slidesOut = new SetExtPose(extState, ArmExtPose.LOW_BASKET);
...
//        telem.addData("CURRENT EXTENSION LENGTH", extState.getRealExtensionLength());
        if (!wristPoseChanged && (extState.getRealExtensionLength() > (ArmExtPose.LOW_BASKET.getLength() - 15) || overrideTimer.time() > 3000)) {
