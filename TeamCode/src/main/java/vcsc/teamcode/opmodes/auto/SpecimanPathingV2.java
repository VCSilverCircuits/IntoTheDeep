...
public class SpecimanPathingV2 extends BaseOpModeAuto {
    ...
    private PathChain prep1, push1, prep2, push2, prep3, push3, score2, grab2, score3, grab3, score4, grab4, score5, park;
    ...
    public void buildPaths() {
        ...
        score2 = follower.pathBuilder()
                .addPath(
                        new BezierLine(
                                new Point(13.682, 21.308, Point.CARTESIAN),
                                new Point(36.500, 68.000, Point.CARTESIAN)
                        )
                )
        ...
        grab2 = follower.pathBuilder()
         ...
         score3 = follower.pathBuilder()
                .addPath(
                        new BezierLine(
                                new Point(13.680, 21.308, Point.CARTESIAN),
                                new Point(36.500, 71.000, Point.CARTESIAN)
                        )
                )
                .setLinearHeadingInterpolation(Math.toRadians(-113), Math.toRadians(180))
                .build();
        ...
        grab3 = follower.pathBuilder()
         ...
         score4 = follower.pathBuilder()
                .addPath(
                        new BezierLine(
                                new Point(13.680, 21.308, Point.CARTESIAN),
                                new Point(36.500, 74.000, Point.CARTESIAN)
                        )
                )
                .setLinearHeadingInterpolation(Math.toRadians(-113), Math.toRadians(180))
                .build();
    ...
    public void loop() {
        ...
        // ===== Park =====
        else if (pathSegment == 25 && (scoreSpecimen.isFinished() || pathTimer.time() > SCORE_DELAY)) {
            scoreSpecimen.cancel();
            clawState.open();
            if (!scoreSpecimen.isFinished()) {
                neutralActionSpecimen.start();
